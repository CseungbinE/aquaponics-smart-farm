import sqlite3
import json
import threading  # [수정] 스레드 락을 위해 추가
import os
from datetime import datetime

class DatabaseManager:
    def __init__(self, config):
        self.lock = threading.Lock()  # [수정] DB 접근 제어를 위한 Mutex Lock 생성
        self.db_path = config.get('database', {}).get('path', 'data/aquaponics.db')
        self.init_db()

    def get_connection(self):
        # [수정] timeout=30.0 추가: DB가 잠겨있을 경우 30초까지 대기 (기본값은 5초)
        return sqlite3.connect(self.db_path, check_same_thread=False, timeout=30.0)

    def init_db(self):
        with self.lock:  # [수정] 초기화 시에도 락 적용
    ...
    def get_latest_reading(self):
        with self.lock:  # [수정] 읽기 작업 락 적용 (API 스레드)
            conn = self.get_connection()
